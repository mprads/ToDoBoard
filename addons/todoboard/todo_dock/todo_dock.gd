...
func build_tree(todos: Dictionary) -> void:
	...
	tree.set_column_custom_minimum_width(1, 16)

	for file: String in todos:
		var file_tree_item := tree.create_item(root)
		file_tree_item.set_text(0, file)
		file_tree_item.add_button(1, FILE, -1, false, "Open File")
		file_tree_item.set_metadata(1, { "path": file, "line": 0 })
		...
		for todo in todos[file]:
			...
			todo_item.add_button(1, EXTERNAL_LINK, -1, false, "Jump to Line")
			todo_item.set_metadata(1, { "path": file, "line": todo.line })
...
func _on_button_clicked(item: TreeItem, column: int, id: int, index: int) -> void:
	...
	base_editor.emit_signal("request_open_script_at_line", load(item.get_metadata(1)["path"]), item.get_metadata(1)["line"])
